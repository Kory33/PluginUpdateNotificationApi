...
public class SemanticPluginVersion extends PluginVersion{
    ...
    private String buildMetadata;
     ...
     public SemanticPluginVersion(String versionString) {
        super(versionString);
        if(versionString.matches("^(0|([1-9][0-9]*))\\.(0|([1-9][0-9]*))\\.(0|([1-9][0-9]*))(\\-[0-9A-Za-z\\.]+)?(\\+[0-9A-Za-z\\.]+)?$") == false)throw new IllegalArgumentException();
        ...
        if(split_versionString[2].matches("^[0-9]+$"))patch =  Integer.parseInt(split_versionString[2]);
        else if(identifier_pos != -1) patch =  Integer.parseInt(split_versionString[2].substring(0,identifier_pos));
        else if(split_versionString[2].indexOf("+") != -1)patch =  Integer.parseInt( split_versionString[2].substring(0, split_versionString[2].indexOf("+")));
        identifier = null;
        buildMetadata = null;
      	String buildMetadata_identifier = null;
      	if(identifier_pos != -1 || versionString.indexOf("+") != -1)buildMetadata_identifier = versionString.substring((identifier_pos != -1?identifier_pos:versionString.indexOf("+"))+1,versionString.length());
        if(buildMetadata_identifier==null)return;
        if(identifier_pos == -1){
        ...
        if(split_identifier.length >= 1 )identifier = split_identifier[0];
        if(split_identifier.length >= 2 )buildMetadata = split_identifier[1];
        return;
    }
    ...
    public boolean isNewerThan(String versionString) {
    	...
    	if(version.identifier == null)return false;
    	if(identifier == null)return version.identifier != null;
    	...
    	for(int i = 0; i < lim; i++){
			...
			else if(i == lim - 1)return version_1.length > version_2.length;
			else continue;
			if (version_1[i].matches("[0-9]+")!=version_2[i].matches("[0-9]+"))return version_1[i].matches("[0-9]+") == false; 
